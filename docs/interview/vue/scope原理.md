...
核心原理：**PostCSS 转换 + 数据属性选择器**。

核心思想是通过为 **DOM 元素** 和 **CSS 选择器** 添加一个 **共同的、唯一的属性标识**，利用 CSS 属性选择器来限制样式的作用范围。

## 工作流程可以分为两个编译阶段：

### A. 模板编译阶段

1. 当 Vue Loader（或 Vite 的 `@vitejs/plugin-vue`）解析 `.vue` 文件时，如果检测到 `<style scoped>`，它会生成一个全局唯一的ID（哈希值）。
