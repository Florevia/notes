...
1. 当你修改了 Vue 中的响应式数据，Vue 不会立即去更新 DOM。
2. Vue 会开启一个异步队列，把所有监听这个数据变化的 Watcher（或 Effect）推入队列中。
3. 如果在同一个事件循环中同一个数据被修改了多次，Vue 会进行去重操作，最终只执行一次 DOM 更新。
...
`nextTick` 中的代码通常会在 **微任务** 阶段执行（在当前宏任务执行完，下一次渲染之前），这样可以确保在浏览器进行 UI 重绘之前更新 DOM，避免不必要的重绘，效率更高。
