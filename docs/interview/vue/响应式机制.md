...
1. Vue 的响应式核心是“依赖收集 + 派发更新”。
2. 在 Vue 2 中，Vue 会通过 `Object.defineProperty` 劫持 `data` 中已有属性的 `getter` 和 `setter`：读取时收集依赖，修改时通知 `watcher` 更新，因此视图会重新渲染。
3. 在 Vue 3 中，响应式系统改成了 `Proxy`，对整个对象做代理，能力更强，能够更自然地监听对象属性的新增、删除和更多操作。
4. `ref`、`reactive`、`computed`、`watch` 底层都建立在这套依赖追踪系统上：访问数据时 `track`，修改数据时 `trigger`
